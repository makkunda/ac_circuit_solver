...
{
    matrix* comp = (matrix*) malloc(sizeof(matrix));
    comp->row = row;
    comp->col = col;
...
matrix *inverse(matrix *b)
{
    ...
    for (i = 0; i < n; i++)
      ...
      for(i = 0; i < n; i++)
      {
        ...
        for(k=0;k<2*n;k++)
        {
            complex *temp11=a->start[i][k];
            complex *temp22=a->start[j][k];
            a->start[i][k]=temp22;
            a->start[j][k]=temp11;
        }

        complex *ratio=getcopy(a->start[i][i]);

        for(j=0;j<2*n;j++)
            a->start[i][j]=divide(a->start[i][j],ratio);

        for(j=0;j<n;j++)
        {
            if(i==j)continue;
            complex* ratio2=getcopy(a->start[j][i]);
            for(k=0;k<2*n;k++)
                a->start[j][k]=sub(a->start[j][k],mult(ratio2,a->start[i][k]));
        }
...
void create_active(char* name,char* n1,char* n2,char* off,char* amp,char* freq,char* del,char* damp,int type){
    ...
    strcat(new1->total," )");
    double delay1 = extract_value(del);
    double pi = 4*atan(1);
    delay1 = -2*pi*delay1;
    complex* n11 = retzero();
